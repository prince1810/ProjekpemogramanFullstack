                    ...
                    <div className="hidden md:flex md:w-1/2 bg-[#003580] text-white p-8 lg:p-10 flex-col justify-between">
                        <div className="flex flex-col gap-4">
                            <div className="flex items-center gap-2">
                                <EAdvocacyLogoSvg />
                                <div>
                                    <h1 className="text-lg font-black text-white leading-tight tracking-wide">AspiraLink</h1>
                                    <p className="text-xs text-blue-200 font-bold">Sistem Keluhan Fasilitas</p>
                                </div>
                            </div>
                            <div className="mt-2">
                                <h2 className="text-3xl font-black text-white leading-tight">Mari Bergabung</h2>
                                <p className="text-blue-100 text-sm mt-2 font-semibold">Buat akun untuk mulai menyampaikan aspirasi demi kemajuan fasilitas kampus STT-NF.</p>
                            </div>
                        </div>
                    ...
                    <div className="w-full md:w-1/2 p-8 flex flex-col items-center justify-center bg-white overflow-y-auto relative">
                        ...
                        <div className="w-full max-w-sm flex flex-col gap-5 mt-8 md:mt-0">
                            <div className="text-center">
                                <h3 className="text-2xl font-black text-[#0b3b84]">Buat Akun Baru</h3>
                                <p className="text-sm font-bold text-gray-400 mt-1">Lengkapi formulir di bawah ini</p>
                            </div>
                            
                            <form onSubmit={handleRegister} className="flex flex-col gap-3">
                                <div className="relative">
                                    <User className="absolute left-4 top-1/2 -translate-y-1/2 text-gray-400 h-4 w-4" />
                                    <input required type="text" placeholder="Nama Lengkap" className="w-full px-10 py-3 text-sm font-bold border border-gray-200 rounded-xl focus:outline-none focus:border-blue-500 focus:ring-2 focus:ring-blue-200" onChange={(e) => setRegData({...regData, nama: e.target.value})} />
                                </div>
                                <div className="relative">
                                    <Hash className="absolute left-4 top-1/2 -translate-y-1/2 text-gray-400 h-4 w-4" />
                                    <input required type="text" placeholder="NIM Mahasiswa" className="w-full px-10 py-3 text-sm font-bold border border-gray-200 rounded-xl focus:outline-none focus:border-blue-500 focus:ring-2 focus:ring-blue-200" onChange={(e) => setRegData({...regData, nim: e.target.value})} />
                                </div>
                                <div className="relative">
                                    <Mail className="absolute left-4 top-1/2 -translate-y-1/2 text-gray-400 h-4 w-4" />
                                    <input required type="email" placeholder="Email Kampus / Aktif" className="w-full px-10 py-3 text-sm font-bold border border-gray-200 rounded-xl focus:outline-none focus:border-blue-500 focus:ring-2 focus:ring-blue-200" onChange={(e) => setRegData({...regData, email: e.target.value})} />
                                </div>
                                <div className="relative">
                                    <Lock className="absolute left-4 top-1/2 -translate-y-1/2 text-gray-400 h-4 w-4" />
                                    <input required type={showPass ? "text" : "password"} placeholder="Buat Password" className="w-full px-10 py-3 text-sm font-bold border border-gray-200 rounded-xl focus:outline-none focus:border-blue-500 focus:ring-2 focus:ring-blue-200" onChange={(e) => setRegData({...regData, password: e.target.value})} />
                                    <button type="button" onClick={() => setShowPass(!showPass)} className="absolute right-4 top-1/2 -translate-y-1/2 text-gray-400 hover:text-blue-600 transition-colors">
                                        {showPass ? <Eye size={18} /> : <EyeOff size={18} />}
                                    </button>
                                </div>
                                <button type="submit" className="w-full mt-3 bg-blue-600 text-white text-base font-black py-3.5 rounded-xl hover:bg-blue-700 shadow-lg shadow-blue-500/30 transition-all hover:-translate-y-1">
                                    Daftar Akun
                                </button>
                            </form>
                            ...
                            <div className="mt-2 text-center text-sm font-bold text-gray-500">
                                Sudah punya akun?{' '}
                                <Link to="/login" className="text-blue-600 hover:text-blue-800 hover:underline transition-colors">
                                    Masuk di sini
                                </Link>
                            </div>
                        </div>

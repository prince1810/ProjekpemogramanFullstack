...
import { User, Upload, Key, ShieldCheck } from "lucide-react";
...
export const ProfilMahasiswa = () => {
  ...
  const handleUpdatePassword = async (e) => {
    ...
    try {
      ...
      Swal.fire("Gagal", err.response?.data?.message || "Cek password lama", "error");
    }
  };
  ...
  const handleUploadPhoto = () => {
    ...
    input.onchange = (e) => {
      ...
      reader.onloadend = async () => {
        ...
        try {
          await axios.put(`http://localhost:3000/api/user/profile/${userId}`, {
            ...studentData,
            avatar: base64,
          });
    ...
    <div className="max-w-3xl mx-auto p-4 space-y-6">
        ...
        <div className="bg-white p-8 rounded-3xl shadow-xl border border-blue-50 relative overflow-hidden">
          ...
          <form onSubmit={handleSaveProfile} className="relative mt-12 space-y-6">
            <div className="flex flex-col items-center">
              <div className="relative group">
                <img
                  src={
                    studentData.avatar
                      ? studentData.avatar.startsWith("data:image") ||
                        studentData.avatar.startsWith("http")
                        ? studentData.avatar
                        : `http://localhost:3000/${studentData.avatar}`
                      : `https://ui-avatars.com/api/?name=${encodeURIComponent(
                          studentData.nama || "User"
                ...
                <button
                  ...
                  className="absolute bottom-0 right-0 bg-white p-2 rounded-full shadow-md text-blue-600"
                >
                  <Upload size={16} />
                </button>
              </div>
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                <label className="text-[10px] uppercase font-bold text-gray-400 ml-1">Nama</label>
                <input
              ...
              <div>
                <label className="text-[10px] uppercase font-bold text-gray-400 ml-1">NIM</label>
                <input
                  className="w-full mt-1 p-3 bg-gray-50 rounded-xl"
              ...
              <div>
                <label className="text-[10px] uppercase font-bold text-gray-400 ml-1">Jurusan</label>
                <input
                  className="w-full mt-1 p-3 bg-gray-50 rounded-xl"
                  value={studentData.jurusan || ""}
                  onChange={(e) => setStudentData({ ...studentData, jurusan: e.target.value })}
                />
              </div>
            </div>
            <button
              type="submit"
              className="w-full bg-[#001f54] text-white py-3 rounded-xl font-bold hover:bg-blue-900 transition"
            >
              Simpan Profil
            </button>
          </form>
      ...
      {activeTab === "keamanan" && (
        <form
          ...
          className="bg-white p-8 rounded-3xl shadow-lg border border-gray-100 space-y-4"
        >

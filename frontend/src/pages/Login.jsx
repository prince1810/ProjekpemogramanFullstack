...
import { Lock, User, EyeOff, Eye, ShieldCheck, MessageCircle, Zap, ArrowLeft } from 'lucide-react';
...
const FeatureItem = ({ icon: Icon, title }) => (
    <div className="flex flex-col items-center justify-center gap-2 flex-1 bg-white/5 p-3 rounded-xl border border-white/10">
        ...
        <h4 className="text-[10px] font-black text-white leading-tight uppercase tracking-wider">{title}</h4>
    </div>
);
...
const Login = () => {
    ...
    const handleLogin = async (e) => {
        ...
        try {
            ...
            if (data.success) {
                if (rememberMe) localStorage.setItem('simkel_remembered_user', loginData.user);
                else localStorage.removeItem('simkel_remembered_user');
                login(data.user);
                setTimeout(() => { data.user.role === 'admin' ? navigate('/dashboard-admin') : navigate('/dashboard-mahasiswa'); }, 100);
            } else {
        ...
        <GoogleOAuthProvider clientId="YOUR_CLIENT_ID">
            ...
            <div className="relative min-h-screen bg-[#0b3b84] font-bubble flex flex-col items-center justify-center p-4 overflow-hidden">
                <div className="absolute inset-0 overflow-hidden pointer-events-none z-0">
                    {[...Array(35)].map((_, i) => <span key={i} className="star" style={{ top: `${Math.random() * 100}%`, left: `${Math.random() * 100}%`, animationDelay: `${Math.random() * 5}s` }} />)}
                </div>
                ...
                <div className="relative z-10 bg-white rounded-3xl shadow-[0_0_60px_rgba(96,165,250,0.4)] border border-blue-300/50 ring-4 ring-blue-400/20 flex flex-col md:flex-row w-full max-w-5xl h-full max-h-[580px] overflow-hidden">
                    ...
                    <div className="hidden md:flex md:w-1/2 bg-[#003580] p-8 lg:p-10 flex-col justify-between text-white">
                        <div className="flex flex-col gap-2">
                            ...
                            <h2 className="text-2xl font-black leading-tight">Jembatan Aspirasi<br/>Kampus</h2>
                    ...
                    <div className="w-full md:w-1/2 p-8 flex flex-col items-center justify-center bg-white relative">
                        ...
                        <div className="w-full max-w-sm flex flex-col gap-5 mt-8 md:mt-0">
                            <div className="text-center">
                                <h3 className="text-2xl font-black text-[#0b3b84]">Masuk ke Akun</h3>
                                <p className="text-sm font-bold text-gray-400 mt-1">Silakan masuk untuk melanjutkan</p>
                            </div>
                            
                            <form onSubmit={handleLogin} className="flex flex-col gap-3">
                                <div className="relative">
                                    <User className="absolute left-4 top-3.5 text-gray-400 h-4 w-4" />
                                    <input type="text" placeholder="Masukkan email atau NIM Anda" className="w-full pl-11 py-3 text-sm font-bold border border-gray-200 rounded-xl focus:border-blue-500 outline-none" value={loginData.user} onChange={(e) => setLoginData({...loginData, user: e.target.value})} />
                                </div>
                                <div className="relative">
                                    <Lock className="absolute left-4 top-3.5 text-gray-400 h-4 w-4" />
                                    <input type={showPass ? "text" : "password"} placeholder="Masukkan password Anda" className="w-full pl-11 pr-11 py-3 text-sm font-bold border border-gray-200 rounded-xl focus:border-blue-500 outline-none" value={loginData.pass} onChange={(e) => setLoginData({...loginData, pass: e.target.value})} />
                                    <button type="button" onClick={() => setShowPass(!showPass)} className="absolute right-4 top-3.5 text-gray-400"><Eye size={18} /></button>
                                </div>
                                <div className="flex justify-between text-xs font-bold text-gray-600">
                                    <label className="flex items-center gap-2 cursor-pointer"><input type="checkbox" checked={rememberMe} onChange={(e) => setRememberMe(e.target.checked)} /> Ingat saya</label>
                                    <a href="#" onClick={handleForgotPassword} className="text-blue-600 hover:underline">Lupa password?</a>
                                </div>
                                <button type="submit" className="w-full bg-blue-600 text-white text-base font-black py-3 rounded-xl hover:bg-blue-700 transition">Masuk</button>
                            </form>
                            
                            <div className="relative flex py-2 items-center"><div className="flex-grow border-t-2 border-dashed border-gray-100"></div><span className="flex-shrink mx-4 text-xs font-bold text-gray-400 uppercase tracking-widest">atau</span><div className="flex-grow border-t-2 border-dashed border-gray-100"></div></div>
                            <div className="flex justify-center"><GoogleLogin onSuccess={handleGoogleSuccess} /></div>
                            <div className="mt-4 text-center text-sm font-bold text-gray-500">Belum punya akun? <Link to="/register" className="text-blue-600 hover:underline">Daftar sekarang</Link></div>
                        </div>
                    </div>
                </div>
            </div>

  ...
  return (
    <div className="font-bubble text-gray-800 bg-gray-50 min-h-screen">
      ...
      <nav className="bg-[#0b3b84] text-white py-4 px-8 flex justify-between items-center sticky top-0 z-50 shadow-md">
        ...
        <Link
          to="/login"
          className="bg-blue-600 hover:bg-blue-500 text-white px-6 py-2 rounded-xl font-extrabold transition shadow"
        >
          Masuk
        </Link>
      </nav>
      ...
      <header
        id="beranda"
        className="relative bg-[#0b3b84] text-white pt-20 pb-28 px-8 rounded-b-[3rem] overflow-hidden flex items-center min-h-[85vh]"
      >
        ...
        <div className="relative z-10 w-full max-w-7xl mx-auto grid md:grid-cols-2 gap-12 lg:gap-8 items-center">
          <div className="flex flex-col justify-center space-y-6 md:pr-4 lg:pr-12">
            ...
            <p className="text-blue-100 text-base md:text-lg max-w-lg leading-relaxed drop-shadow-md font-semibold tracking-wide">
              Laporkan keluhan fasilitas, berikan saran, dan pantau perkembangan
              laporan Anda secara real-time.
            </p>
            <div className="flex flex-wrap gap-4 pt-4">
              <Link
                to="/login"
                className="bg-blue-500 hover:bg-blue-400 text-white px-8 py-3.5 rounded-2xl font-extrabold shadow-lg shadow-blue-500/30 transition-all hover:-translate-y-1 flex items-center gap-2 text-sm md:text-base"
              >
                ✍️ Buat Laporan
              </Link>
              <a
                href="#carakerja"
                className="border-2 border-white/80 hover:bg-white hover:text-[#0b3b84] text-white px-8 py-3.5 rounded-2xl font-extrabold transition-all hover:-translate-y-1 flex items-center gap-2 text-sm md:text-base"
              >
                ▶ Lihat Cara Kerja
              </a>
            </div>
            <div className="flex items-center gap-2 pt-6">
              <span className="flex items-center justify-center w-6 h-6 rounded-full bg-blue-400/20 text-blue-200">
                🛡️
              </span>
      ...
      <section id="fitur" className="py-20 px-8 max-w-7xl mx-auto text-center">
        <h3 className="text-blue-600 font-extrabold uppercase tracking-wider text-sm mb-2">
          Fitur Utama
        </h3>
        <h2 className="text-3xl font-black mb-12 text-[#0b3b84]">
          Kenapa Memilih AspiraLink?
        </h2>
        <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
          <FeatureCard
            icon="📱"
            title="Pelaporan Mudah"
            desc="Laporkan kerusakan atau masalah fasilitas hanya dalam beberapa langkah praktis."
          />
          <FeatureCard
            icon="🛡️"
            title="Aman dan Rahasia"
            desc="Identitas pelapor terlindungi dengan sistem keamanan dan enkripsi data."
          />
          <FeatureCard
            icon="📊"
            title="Tracking Status"
            desc="Pantau perkembangan laporan secara real-time dari proses hingga selesai."
          />
          <FeatureCard
            icon="⚡"
            title="Respon Cepat"
            desc="Laporan diteruskan ke pihak terkait secara otomatis untuk penanganan lebih cepat."
          />
        </div>
      </section>
      ...
      <section id="carakerja" className="py-20 px-8 bg-white text-center">
        ...
        <div className="flex flex-col md:flex-row justify-center items-center md:items-start max-w-6xl mx-auto gap-4">
          <StepItem
            ...
            icon="👤"
            title="Login"
            desc="Login menggunakan akun kampus Anda."
          />
          <StepDivider />
          <StepItem
            number="2"
            icon="📝"
            title="Buat Laporan"
            desc="Buat laporan keluhan atau aspirasi fasilitas."
          />
          <StepDivider />
          <StepItem
            number="3"
            icon="✅"
            title="Verifikasi"
            desc="Admin memverifikasi laporan yang masuk."
          />
          <StepDivider />
          <StepItem
            number="4"
            ...
            desc="Laporan diteruskan ke unit terkait."
          />
          <StepDivider />
          <StepItem
            number="5"
            icon="🔄"
            title="Update Status"
            desc="Status laporan diperbarui secara berkala."
          />
          <StepDivider />
          <StepItem
            number="6"
            icon="🎉"
            title="Selesai"
            desc="Laporan selesai dan ditutup."
          />
        </div>
      </section>
      ...
      <footer className="bg-[#0b3b84] text-white py-6 text-center font-bold">
        <p className="text-sm">
          © 2026 AspiraLink - Sistem Keluhan Fasilitas Kampus. All rights
          reserved.
        </p>
      </footer>
    </div>
...
const StatCard = ({ number, label, icon, color }) => (
  <div className="bg-white p-6 rounded-[2rem] shadow-sm border border-gray-100 flex items-center gap-4 text-left">
    <div className={`text-4xl bg-gray-50 p-4 rounded-3xl ${color}`}>{icon}</div>
    <div>
